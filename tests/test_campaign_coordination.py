...
check("Export has children", len(tree.get("children", [])) > 0 or len(tree.get("items", [])) > 0)
...
check("Export items are enriched with IDs", first_item_id is not None, f"id={first_item_id}")
...
status, data = req("POST", f"/api/campaigns/{cid}/join", {
...
if first_item_id:
    # Claim the item
    status, data = req("POST", f"/api/campaigns/{cid}/items/{first_item_id}/claim", {
        "instance_id": FAKE_PEER,
        "hostname": FAKE_HOST,
    })
    ...
    check("Claimed item shows claimed_by", (claimed_item or {}).get("claimed_by") == FAKE_PEER)
    check("Claimed item shows hostname",
...
if first_item_id:
    status, data = req("POST", f"/api/campaigns/{cid}/items/{first_item_id}/status", {
        "status": "in_progress",
    })
    ...
    check("Item status updated to in_progress",
...
if first_item_id:
    ...
    check("Unclaimed item has claimed_by=None",
...
if first_item_id:
    req("POST", f"/api/campaigns/{cid}/items/{first_item_id}/status", {
        "status": "planned",
        "outcome": None,
    })
    # Remove participant
