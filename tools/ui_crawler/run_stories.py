...
def _slim(meta):
    return {"id": meta["id"], "title": meta["title"], "cluster": meta.get("cluster", ""),
...
def _diff(results):
    ...
    flips = [(r["id"], base[r["id"]]["status"], r["status"], r["title"])
             for r in results if r["id"] in base and base[r["id"]]["status"] != r["status"]]
...
def _status_md(results):
    n = {k: sum(1 for r in results if r["status"] == k) for k in ("works", "partial", "gap", "blocked")}
    lines = ["# Gently — UX audit status", "",
...
def main():
    ...
    ap.add_argument("--account-url", default=None, help="account-mode (logged-out) server for view-only stories")
    ap.add_argument("--out", default="tools/ui_crawler/out/stories")
    ap.add_argument("--headed", action="store_true")
    ap.add_argument("--slow-mo", type=int, default=0)
    ap.add_argument("--update-baseline", action="store_true", help="write the current run as the committed baseline")
    ap.add_argument("--docs-status", action="store_true", help="also write docs/user-stories/STATUS.md")
    args = ap.parse_args()
