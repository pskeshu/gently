...
def fp_hash(fp: dict) -> str:
    key = json.dumps(
        {k: fp.get(k) for k in ("path", "landingVisible", "activeTab", "activeViews", "panels", "modal")},
        sort_keys=True,
    )
...
class Crawler:
    ...
    async def _probe(self, context, state, element):
        ...
        async with self._sem:
            ...
            page.on("response", lambda r: http_errors.append(f"{r.status} {r.url.split('?')[0]}")
                    if r.status >= 400 else None)
            edge = {"from": state["hash"], "from_label": fp_label(state["fp"]),
                    "via": element["selector"], "via_text": element["text"], "via_kind": element["kind"]}
            try:
                ...
                new_toast = [t for t in (after.get("toast") or []) if t not in (before.get("toast") or [])]
                edge.update({
                    "result": "ok",
                    "to": fp_hash(after),
                    ...
                    "returned_to_landing": bool(after.get("landingVisible")) and not before.get("landingVisible"),
                })
                new_state = None
                if changed and len(state["path"]) + 1 <= self.max_depth:
                    elems = await self._enumerate(page)
                    new_state = {"hash": edge["to"], "path": state["path"] + [element["selector"]],
                                 "fp": after, "elements": elems}
    ...
    async def run(self):
        async with async_playwright() as p:
            ...
            while frontier and len(self.states) < self.max_states:
                ...
                print(f"[crawl] state {fp_label(state['fp'])} ({sh}) depth={len(state['path'])} "
                      f"probing {len(elems)} elements  [{len(self.states)} states so far]", flush=True)
    ...
    def analyse(self) -> dict:
        ...
        dead = [e for e in ok if not e["changed"] and not e["toast"]
                and not e["console_errors"] and not e["http_errors"]]
        ...
        return {"dead": dead, "console_err": console_err, "http_err": http_err,
                "spinners": spinners, "to_landing": to_landing, "unreachable_tabs": unreachable}

    def write(self):
        self.out.mkdir(parents=True, exist_ok=True)
        (self.out / "graph.json").write_text(json.dumps(
            {"states": self.states, "edges": self.edges}, indent=2))
        a = self.analyse()
        ...
        for e in self.edges:
            ...
            mark = " ⚑" if e["returned_to_landing"] or e["http_errors"] or e["console_errors"] else ""
        ...
        def block(title, items, fmt):
            ...
            r.append("")
            return r
        rep = [f"# Gently UI crawl — {self.url}", "",
               f"States discovered: **{len(self.states)}** · edges probed: **{len(self.edges)}** · "
               f"engine: {self.browser_name}", ""]
        rep += block("↩ Transitions that return to the landing", a["to_landing"],
                     lambda e: f"- from **{e['from_label']}** via `{e['via_text'] or e['via_kind']}` "
                               f"(`{e['via']}`) → landing")
        rep += block("💥 Clicks that triggered console errors", a["console_err"],
                     lambda e: f"- **{e['from_label']}** · `{e['via_text']}` → {e['console_errors']}")
        rep += block("🌐 Clicks that triggered HTTP 4xx/5xx", a["http_err"],
                     lambda e: f"- **{e['from_label']}** · `{e['via_text']}` → {e['http_errors']}")
        rep += block("⏳ Clicks that left a spinner running", a["spinners"],
                     lambda e: f"- **{e['from_label']}** · `{e['via_text']}` → spinner still visible")
        rep += block("🚫 Dead controls (no state change / toast / error / nav)", a["dead"],
                     lambda e: f"- **{e['from_label']}** · `{e['via_text'] or e['via_kind']}` (`{e['via']}`)")
        rep += ["## Tabs enumerated but never reachable as active", "",
