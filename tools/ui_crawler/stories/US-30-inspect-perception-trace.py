...
META = {"id": "US-30", "title": "Inspect a perception trace / VLM reasoning",
...
async def flow(page, url, rec):
    ...
    if panel and rail:
        rec.partial("Embryos → Default shows the per-embryo rail + VLM reasoning panel; detection cards render 'Show VLM reasoning' with classifier/perceiver text, raw trace JSON and 'View All Projections' — all populated by a live perception run (empty state 'Select an embryo to view analysis history' without one)")
    elif panel:
