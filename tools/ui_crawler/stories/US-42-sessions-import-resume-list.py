...
META = {"id": "US-42", "title": "Sessions: list, resume, import",
...
async def flow(page, url, rec):
    ...
    imp = await count_text(page, r"import")          # session import (from file/peer) — expected absent here
    await rec.shot("sessions-list")
    if listed and resume:
        rec.ok(f"Sessions tab lists sessions with a per-session Resume ({resume}); no session import-from-file/peer affordance (import={imp})")
    elif listed:
        rec.partial(f"session list surface present but no Resume to exercise headless (no session data; resume renders per non-active session); no import-from-file/peer affordance (import={imp})")
    else:
