...
META = {"id": "US-14", "title": "Start an adaptive timelapse",
        "cluster": "7 Timelapse", "mode": "rig", "needs_account": False}
...
async def flow(page, url, rec):
    ...
    await page.evaluate("() => { const r=document.getElementById('op-rail'); if (r) r.dataset.active='c0'; }")  # reveal the gated run-chooser step for the audit shot
    ...
    if surface and adaptive and interval and stopc and start:
        rec.blocked(f"needs device/session: adaptive-timelapse chooser present (mode radio + interval/stop select + 'Start run', start={start}) but the run-chooser step (op-rail c0) only unlocks after embryos are marked on live hardware, and /api/devices/timelapse/start 502s with the device layer offline")
    else:
        rec.gap(f"adaptive-timelapse start control incomplete (surface={surface}, adaptive={adaptive}, interval={interval}, stop={stopc}, start={start})")
