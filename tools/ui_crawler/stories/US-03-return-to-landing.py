...
META = {"id": "US-03", "title": "Return to the landing after dismissing it",
...
async def flow(page, url, rec):
    ...
    if reshown:
        rec.partial(f"header brand 'Gently Microscopy' → / re-shows the landing — functional but incidental (a logo click, not a labelled 'back to start'; dismissed-before={dismissed})")
    else:
