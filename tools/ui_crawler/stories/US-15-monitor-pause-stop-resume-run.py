...
META = {"id": "US-15", "title": "Monitor / pause / stop / resume a run",
...
async def flow(page, url, rec):
    ...
    await page.evaluate("() => { const r=document.getElementById('op-rail'); if (r) r.dataset.active='running'; }")  # reveal the gated run-spine step for the audit shot
    ...
    if surface and spine and pause and stop:
        rec.blocked(f"needs live run: run-spine monitor + Pause/Resume toggle + Stop present (pause={pause}, stop={stop}) but the running step (op-rail running) only appears once a timelapse is live, and pause/resume/stop hit /api/devices/timelapse/{{pause,resume,stop}} which 502 with the device layer offline")
    else:
        rec.gap(f"run-spine controls incomplete (surface={surface}, spine={spine}, pause={pause}, stop={stop})")
