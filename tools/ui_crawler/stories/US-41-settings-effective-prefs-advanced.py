...
META = {"id": "US-41", "title": "Settings: effective config / prefs / advanced",
...
async def flow(page, url, rec):
    ...
    prefs = await exists(page, "#pref-export") and await exists(page, "#pref-import") and await exists(page, "#pref-save-defaults")
    ...
    if effective and prefs and advanced:
        rec.ok("Settings exposes Effective config (read-only server config), dashboard prefs (export/import/save-as-rig-defaults/reset), and Advanced (restart-required) tunables")
    elif effective or prefs or advanced:
        rec.partial(f"only some config surfaces present (effective={effective}, prefs={prefs}, advanced={advanced})")
    else:
