...
META = {"id": "US-05", "title": "Review & commit a designed plan",
...
async def flow(page, url, rec):
    ...
    export = await dom_count(page, "#v2-plan-export")         # export the finished plan (wired but hidden until plan-ready)
    await rec.shot("plan-review-panel")
    if side and cont and export:
        rec.blocked(f"needs live agent: plan review panel + commit controls present (continue={cont}, export-wired={export>0}) but the panel is {'empty' if empty else 'unpopulated'} — no plan to review/commit until a /plan turn authors one")
    else:
        rec.gap(f"plan review/commit surface incomplete (side={side}, continue={cont}, export={export})")
