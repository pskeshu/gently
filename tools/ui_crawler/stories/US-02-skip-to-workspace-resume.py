...
META = {"id": "US-02", "title": "Skip to workspace and resume a prior session",
...
async def flow(page, url, rec):
    ...
    resume = await dom_count(page, ".session-resume-btn")   # "Resume in agent" per non-active session
    await rec.shot("sessions-list")
    if had_landing and dismissed and on_sessions and resume:
        rec.ok(f"skip → workspace; sessions tab lists {items} session(s) with {resume} Resume control(s)")
    elif dismissed and on_sessions and listbox:
        rec.partial(f"skip→workspace + sessions list present, but no resumable prior session in dev data (items={items}, resume={resume})")
    else:
        rec.gap(f"skip/resume path broken (landing={had_landing}, dismissed={dismissed}, sessions_tab={on_sessions}, list={listbox})")
