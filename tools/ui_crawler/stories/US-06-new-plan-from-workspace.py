...
META = {"id": "US-06", "title": "Start a new plan from the workspace",
...
async def flow(page, url, rec):
    ...
    newplan = await count_text(page, r"new plan|create plan|\+\s*plan|new campaign|create campaign|start planning")
    if newplan:
        rec.ok(f"workspace exposes a labelled plan-creation control ({newplan})")
    else:
        rec.gap("no labelled 'New plan' control in the workspace/plans tab — planning is only reachable via the header-logo→landing reset or agent chat")
