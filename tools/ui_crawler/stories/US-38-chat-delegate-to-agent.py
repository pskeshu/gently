...
META = {"id": "US-38", "title": "Chat / delegate to the agent",
...
async def flow(page, url, rec):
    ...
    if opened and text and send:
        rec.partial("toggle opens the docked chat panel with composer (textarea + Send); delegating a task + getting a reply needs a live agent turn")
    elif opened:
