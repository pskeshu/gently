...
META = {"id": "US-16", "title": "View the Operation Plan",
...
async def flow(page, url, rec):
    ...
    if title and spine and nodes >= 2:
        rec.ok(f"Operations tab renders the plan: title + tactic spine with {nodes} tactic cards (via audit scenario fixture; live needs a running experiment)")
    elif spine or nodes:
        rec.partial(f"spine present but plan render is thin (title={title}, spine={spine}, nodes={nodes})")
    else:
