...
META = {"id": "US-13", "title": "Run chooser + assign roles after marking",
        "cluster": "6 Operate (acquire)", "mode": "headless", "needs_account": False}
...
async def flow(page, url, rec):
    ...
    await page.evaluate("() => { if (typeof ClientEventBus !== 'undefined') ClientEventBus.emit('EMBRYOS_UPDATE', {embryos:[{id:'embryo_1'},{id:'embryo_2'},{id:'embryo_3'}]}); }")
    ...
    modes = await dom_count(page, 'input[name="op-mode"]')       # run modes (manual/adaptive/library/plan/agent)
    start = await exists(page, "#op-run-start")
    await rec.shot("run-chooser")
    if chooser and chips >= 2 and modes >= 2 and start:
        rec.ok(f"Run chooser appears after marking: {chips} role chips (toggle subject/reference) + {modes} run modes + Start run")
    elif chooser:
