...
META = {"id": "US-33", "title": "See learnings / observations",
...
async def flow(page, url, rec):
    ...
    if opened and obs and find and notes:
        rec.partial("Notebook tab surfaces learnings: Observation/Finding/Question filters + notes pane present, but the pane fills only as the agent records notes (no live data headless)")
    elif opened and notes:
        rec.partial(f"notebook notes pane present but kind filters incomplete (observations={obs}, findings={find})")
    else:
        rec.gap("no notebook/learnings surface reachable — observations & findings have nowhere to be read")
