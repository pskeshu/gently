...
META = {"id": "US-08", "title": "Take a quick look jumps straight to the scope",
        "cluster": "4 Standalone", "mode": "headless", "needs_account": False}
...
async def flow(page, url, rec):
    ...
    if tab_active or content_active:
        rec.ok("'Take a quick look' dismisses the landing and activates the Devices (scope) tab — no plan required")
    elif card:
