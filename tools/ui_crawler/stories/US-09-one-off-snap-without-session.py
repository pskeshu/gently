...
META = {"id": "US-09", "title": "One-off snap/volume without a session",
        "cluster": "4 Standalone", "mode": "rig", "needs_account": False}
...
async def flow(page, url, rec):
    ...
    rec.blocked(f"needs device: Snap Volume{'/Burst' if burst else ''} present but POST /api/devices/acquire/volume 502s with the device layer offline; the failure is SILENT — only console.error, no toast/UI feedback (toast_visible={toast})")
