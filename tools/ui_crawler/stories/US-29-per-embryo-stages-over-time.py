...
META = {"id": "US-29", "title": "Per-embryo predicted stages over time",
...
async def flow(page, url, rec):
    ...
    if vitals_btn and board_btn and vitals and board:
        rec.partial("Embryos tab exposes Board (per-embryo stage sparkline column) + Vitals (per-embryo stage strip charts across timepoints); the stage-over-time data populates from a live perception run — empty state ('No embryos to display') without one")
