...
META = {"id": "US-25", "title": "Configure the thermalizer (serial/MQTT) in Settings",
...
async def flow(page, url, rec):
    ...
    backends = await dom_count(page, 'input[name="th-backend"]')   # radios are display:none, styled labels
    apply = await exists(page, "#th-apply")
    test = await exists(page, "#th-test")
    if section and backends >= 2 and apply and test:
        rec.ok(f"Settings → Hardware → Thermalizer: {backends} backend options + Test + Apply present")
    elif section:
        rec.partial(f"thermalizer section present but controls incomplete (backends={backends}, apply={apply}, test={test})")
    else:
