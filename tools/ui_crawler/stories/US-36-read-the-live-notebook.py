...
META = {"id": "US-36", "title": "Read the live notebook", "cluster": "12 Notebook", "mode": "headless", "needs_account": False}
...
async def flow(page, url, rec):
    ...
    if surface and filters >= 2 and cards:
        rec.ok(f"notebook tab renders {cards} entries + kind filter ({filters} kinds) + thread rail")
    elif surface and filters >= 2:
        rec.partial(f"notebook reading surface renders (kind filter={filters}, thread rail, notes region) but no live notes in dev — empty state shown")
    else:
