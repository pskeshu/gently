...
META = {"id": "US-39", "title": "Answer an agent question (ask flow)",
...
async def flow(page, url, rec):
    ...
    if await exists(page, "#agent-chat-toggle"):      # asks are dual-rendered on the stage AND in the chat log
    ...
    live_ask = await exists(page, "#ask-stage .ac-choice") # a rendered pending-ask card with choice buttons
    ...
    if not stage:
        ...
        rec.blocked("needs live agent: no pending ask — #ask-stage stays hidden until the agent asks a question")
