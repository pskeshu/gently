...
META = {"id": "US-10", "title": "Mark embryos (bottom-cam detect → single list)",
        "cluster": "5 Operate (mark)", "mode": "headless", "needs_account": False}
...
async def flow(page, url, rec):
    ...
    detect = await dom_count(page, "#op-detect")            # a2 group: Detect (SAM) — hidden until marking active
    ...
    if opened and tomark and board and mark_step and detect and confirm:
        rec.partial("operate view exposes the marking entry (#op-tomark), SAM detect + confirm-into-list controls, and one canonical worklist (#op-board); capturing a FOV + placing/detecting markers needs the bottom-cam device (offline → 502)")
    elif opened and board:
        rec.partial(f"operate surface present but a marking affordance is missing (tomark={tomark}, detect={detect}, confirm={confirm})")
    else:
        rec.gap("operate/marking surface not reachable — no bottom-cam marking UI or single embryo worklist")
