...
META = {"id": "US-26", "title": "Set a temperature setpoint from Devices",
        "cluster": "9 Temperature", "mode": "rig", "needs_account": False}
...
async def flow(page, url, rec):
    ...
    if tinput and tset:
        rec.blocked(f"needs device: setpoint input + Set button wired in the Devices header (input={tinput}, set={tset}), but #devices-temp stays hidden until the ACUITYnano controller is online (revealed={revealed}) and /api/devices/temperature/set 502s with the device layer offline")
    else:
