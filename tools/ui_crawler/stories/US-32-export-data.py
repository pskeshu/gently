...
META = {"id": "US-32", "title": "Export data",
...
async def flow(page, url, rec):
    ...
    if emb + pln + gal:
        rec.ok(f"a data export/download control is present (embryos={emb}, plans={pln}, gallery={gal})")
    else:
        rec.gap("no export/download-data control across embryos, plans, or gallery — the only exports are the plan-markdown button (landing overlay) and a dashboard-prefs JSON dump (settings); acquired volumes/predictions/traces/results cannot be exported from the UI")
