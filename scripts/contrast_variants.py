...
def main():
    for embryo, tp in CASES:
        ...
        print(f"\n{embryo} t{tp:04d}: shape={proj.shape} min={proj.min()} max={proj.max()} p99={np.percentile(proj,99):.0f}")
        for recipe in RECIPES:
            out_img = preprocess(proj, recipe)
            p = OUT / f"{embryo}_t{tp:04d}_{recipe}.png"
            Image.fromarray(out_img).save(p)
            print(f"  {recipe:<15}  display p50={np.percentile(out_img,50):.0f} p90={np.percentile(out_img,90):.0f} max={out_img.max()}")
