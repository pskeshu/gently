...
def _build_perception_tools() -> list:
    ...
    # Normal progression stages only (exclude arrested, no_object)
    stage_values = organism.STAGES
    return [
    {
    ...
    },
    {
        "name": "clip_volume",
        "description": (
            "Clip the current embryo's 3D volume with an arbitrarily-oriented 3D box and view "
            "a projection. This is ported from BigVolumeBrowser's clipping feature.\n\n"
            "You can cut the volume along ANY plane — not just axis-aligned. Use ranges to define "
            "the box size, and rotation to orient it at any angle. This lets you:\n"
            "- Peel away outer layers to reveal internal folding and coiling\n"
            "- Cut at an angle matching the embryo's body axis\n"
            "- Isolate a single body segment by rotating the clip box\n"
            "- Take oblique slices to see structures hidden in orthogonal views\n\n"
            "Ranges are fractions (0.0-1.0) of each axis dimension.\n\n"
            "USE THIS WHEN:\n"
            "- You suspect folding or coiling but can't see it in the full projection\n"
            "- You need to distinguish 1.5-fold from 2-fold (count body layers)\n"
            "- You need to see if the body has doubled back on itself\n"
            "- The pretzel stage's tight coiling is ambiguous in full projection\n"
            "- The embryo is oriented at an angle and you need an oblique cut\n\n"
            "Example inputs:\n"
            "- {\"z_min\": 0.3, \"z_max\": 0.7, \"reason\": \"Remove outer layers to see internal folding\"}\n"
            "- {\"z_min\": 0.2, \"z_max\": 0.8, \"rotate_x\": 30, \"reason\": \"Oblique cut along embryo body axis\"}\n"
            "- {\"y_min\": 0.0, \"y_max\": 0.5, \"rotate_z\": 45, \"reason\": \"Diagonal cut to isolate head region\"}"
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "z_min": {
                    "type": "number",
                    "description": "Start of Z clip range (0.0=top, 1.0=bottom). Controls depth slicing.",
                    "minimum": 0.0,
                    "maximum": 1.0,
                },
                "z_max": {
                    "type": "number",
                    "description": "End of Z clip range (0.0=top, 1.0=bottom).",
                    "minimum": 0.0,
                    "maximum": 1.0,
                },
                "y_min": {
                    "type": "number",
                    "description": "Start of Y clip range (0.0=top edge, 1.0=bottom edge).",
                    "minimum": 0.0,
                    "maximum": 1.0,
                },
                "y_max": {
                    "type": "number",
                    "description": "End of Y clip range.",
                    "minimum": 0.0,
                    "maximum": 1.0,
                },
                "x_min": {
                    "type": "number",
                    "description": "Start of X clip range (0.0=left edge, 1.0=right edge).",
                    "minimum": 0.0,
                    "maximum": 1.0,
                },
                "x_max": {
                    "type": "number",
                    "description": "End of X clip range.",
                    "minimum": 0.0,
                    "maximum": 1.0,
                },
                "rotate_x": {
                    "type": "number",
                    "description": "Rotate the clip box around X axis in degrees (-90 to 90). Tilts the cutting plane.",
                    "minimum": -90,
                    "maximum": 90,
                },
                "rotate_y": {
                    "type": "number",
                    "description": "Rotate the clip box around Y axis in degrees (-90 to 90).",
                    "minimum": -90,
                    "maximum": 90,
                },
                "rotate_z": {
                    "type": "number",
                    "description": "Rotate the clip box around Z axis in degrees (-90 to 90).",
                    "minimum": -90,
                    "maximum": 90,
                },
                "center_z": {
                    "type": "number",
                    "description": "Z center of the clip box (0.0-1.0). Default: center of z_range.",
                    "minimum": 0.0,
                    "maximum": 1.0,
                },
                "center_y": {
                    "type": "number",
                    "description": "Y center of the clip box (0.0-1.0). Default: center of y_range.",
                    "minimum": 0.0,
                    "maximum": 1.0,
                },
                "center_x": {
                    "type": "number",
                    "description": "X center of the clip box (0.0-1.0). Default: center of x_range.",
                    "minimum": 0.0,
                    "maximum": 1.0,
                },
                "projection": {
                    "type": "string",
                    "description": "Projection method: 'max' (default MIP), 'mean' (average), or 'three_view' (XY+YZ+XZ).",
                    "enum": ["max", "mean", "three_view"],
                },
                "axis": {
                    "type": "integer",
                    "description": "Projection axis: 0=Z (top-down), 1=Y (front), 2=X (side). Default 0. Ignored for three_view.",
                    "minimum": 0,
                    "maximum": 2,
                },
                "reason": {
                    "type": "string",
                    "description": "Brief explanation of what you're looking for inside the volume",
                },
            },
            "required": ["reason"],
        },
...
_PERCEPTION_TOOLS_CACHE = None


def _get_perception_tools() -> list:
    """Return perception tools, building them on first call."""
    global _PERCEPTION_TOOLS_CACHE
    if _PERCEPTION_TOOLS_CACHE is None:
        _PERCEPTION_TOOLS_CACHE = _build_perception_tools()
    return _PERCEPTION_TOOLS_CACHE


# Backward-compatible name for any external references
PERCEPTION_TOOLS = None  # Use _get_perception_tools() instead
...
class PerceptionEngine:
    ...
    def _handle_tool_call(
        ...
        # Reference images are already included in the prompt, no need for tools
        ...
        elif tool_name == "clip_volume":
            z_min = tool_input.get("z_min")
            z_max = tool_input.get("z_max")
            y_min = tool_input.get("y_min")
            y_max = tool_input.get("y_max")
            x_min = tool_input.get("x_min")
            x_max = tool_input.get("x_max")
            rotate_x = tool_input.get("rotate_x", 0)
            rotate_y = tool_input.get("rotate_y", 0)
            rotate_z = tool_input.get("rotate_z", 0)
            center_z = tool_input.get("center_z")
            center_y = tool_input.get("center_y")
            center_x = tool_input.get("center_x")
            projection = tool_input.get("projection", "max")
            axis = tool_input.get("axis", 0)
            reason = tool_input.get("reason", "")

            z_range = (z_min, z_max) if z_min is not None or z_max is not None else None
            ...
            if z_range and z_range[0] is None:
                z_range = (0.0, z_range[1])
            if z_range and z_range[1] is None:
                z_range = (z_range[0], 1.0)
            if y_range and y_range[0] is None:
                y_range = (0.0, y_range[1])
            if y_range and y_range[1] is None:
                y_range = (y_range[0], 1.0)
            if x_range and x_range[0] is None:
                x_range = (0.0, x_range[1])
            if x_range and x_range[1] is None:
                x_range = (x_range[0], 1.0)

            # Build rotation and center tuples
            rotation = None
            if any(abs(a) > 0.01 for a in (rotate_x, rotate_y, rotate_z)):
                rotation = (rotate_x, rotate_y, rotate_z)

            center = None
            if any(v is not None for v in (center_z, center_y, center_x)):
                center = (
                    center_z if center_z is not None else 0.5,
                    center_y if center_y is not None else 0.5,
                    center_x if center_x is not None else 0.5,
                )
            ...
            if self._current_volume is not None:
                ...
                vol = self.volume_accessor(self._current_embryo_id, timepoint)

            if vol is not None:
                range_desc = []
                if z_range:
                    range_desc.append(f"Z=[{z_range[0]:.0%}-{z_range[1]:.0%}]")
                if y_range:
                    range_desc.append(f"Y=[{y_range[0]:.0%}-{y_range[1]:.0%}]")
                if x_range:
                    range_desc.append(f"X=[{x_range[0]:.0%}-{x_range[1]:.0%}]")
                clip_desc = ", ".join(range_desc) if range_desc else "full volume"
                if rotation:
                    clip_desc += f", rot=({rotate_x},{rotate_y},{rotate_z})°"

                logger.info(
                    f"Tool: Clipping volume {clip_desc}, "
                    f"projection={projection}, axis={axis}"
                )

                rendered_b64 = clip_and_project(
                    vol,
                    z_range=z_range,
                    y_range=y_range,
                    x_range=x_range,
                ...
                content = [
                    {
                        "type": "text",
                        "text": (
                            f"Clipped volume view at T{timepoint} "
                            f"(clip: {clip_desc}, projection={projection}, axis={axis}). "
                            f"Original volume shape: {vol_shape[0]}x{vol_shape[1]}x{vol_shape[2]} (ZxYxX). "
                            f"Requested because: {reason}"
                        ),
                    },
                    {
                ...
                summary = f"Clipped {clip_desc} ({projection})"
                return content, summary, timepoint, "clip_volume"
            else:
                content = [
                    {
                        "type": "text",
                        "text": "3D volume not available. Clipping requires volume data to be passed to perceive().",
                    }
                ]
                summary = "Volume not available"
                return content, summary, None, None
    ...
    def _build_cached_system_prompt(self) -> List[Dict]:
        ...
        The prompt text is pulled from the active organism module so that
        swapping organisms automatically changes VLM instructions.
        """
        organism = get_organism()
        return [{
            "type": "text",
