...
# Default optical parameters for bottom camera
...
# This function name was misleading - it was named for movement but used for position
# TODO: Remove after all callers are updated
def pixel_to_stage_offset(
    pixel_offset_x: float,
    pixel_offset_y: float,
    pixel_size_um: float,
    invert_x: bool = False  # Changed default to False - callers must be explicit
) -> Tuple[float, float]:
    """
    DEPRECATED: Use pixel_to_stage_position() or pixel_displacement_to_stage_movement() instead.

    This function exists only for backwards compatibility.
    The function name was confusing - it could mean either position or movement.

    Parameters
    ----------
    pixel_offset_x, pixel_offset_y : float
        Pixel offsets from some reference point
    pixel_size_um : float
        Effective pixel size in micrometers
    invert_x : bool
        If True, inverts X axis (use for MOVEMENT calculations)
        If False, no inversion (use for POSITION calculations)

    Returns
    -------
    Tuple[float, float]
        (dx_stage, dy_stage) in micrometers
    """
    import warnings
    warnings.warn(
        "pixel_to_stage_offset() is deprecated. Use pixel_to_stage_position() "
        "for position calculations or pixel_displacement_to_stage_movement() "
    ...
    if invert_x:
        dx_stage = -pixel_offset_x * pixel_size_um
    else:
        dx_stage = pixel_offset_x * pixel_size_um
